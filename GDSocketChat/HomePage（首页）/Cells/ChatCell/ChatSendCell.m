...
//    CGSize size = [ChatManager get_Recv_ChatCellHeightWithMessage:model.chat_message];
    ...
    [self.messageLabel mas_updateConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.chatImgView).offset(10);
        make.right.equalTo(self.chatImgView).offset(-15);
        make.top.equalTo(self.chatImgView).offset(10);
        make.bottom.equalTo(self.chatImgView).offset(-10);
    }];
...
- (UIImageView *)chatImgView {
    if (!_chatImgView) {
        ...
        UIImage *img = GDImage(@"chat_send_normal");
